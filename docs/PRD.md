...
**Current release:** Local-only IDE sidebar, hybrid rule-based step explanations, LLM-written turn recaps when a model is available, ~102 glossary terms, ~57 curated concepts, ~40 recommendation catalog entries, Cursor hooks plugin for richer data capture, 81 unit tests.

---

## 2. Problem statement

When Cursor's agent works on a task, the chat log is dense: shell commands, file paths, tool names, and jargon. For users with little coding background, this reads like hieroglyphs. They cannot reliably answer:
...
- Uses Cursor to build or modify projects with agent help
...
├── Optional banner: Install Cursor hooks
...
## 6. Feature requirements (as shipped in v0.2)
...
| `working` | Agent turn still active | Watching the agent work |
...
**Operational note:** After building from source, run `npm run install:cursor` in `extension/` and **Developer: Reload Window**. Cursor does not hot-reload extension UI changes; an stale install will still show the pre-overlay behavior (full-panel replace loader, top summary strip).
...
| Requirement | Status | Notes |
|-------------|--------|-------|
| Rule-based templates | Shipped | ~60 patterns: git, ripgrep, npm/pnpm, dev servers, file ops, Cursor tools |
...
**Description:** Companion plugin capturing data transcripts lack.
...
| Per-turn cache | Shipped | `~/.cursor/projects/<slug>/decipher/research/` |
...
Provider: VS Code Language Model API (`vscode.lm`). Graceful fallback to `composeTurnRecap` when unavailable. Redacted input only — never file contents.
...
| Decipher: Install Cursor hooks | Copy plugin locally |
...
1. User starts a Cursor agent chat
...
3. Sees hooks banner → clicks **Install** → reloads
...
│  Cursor IDE                                              │
│  ┌──────────────┐    ┌─────────────────────────────────┐ │
│  │ Agent chat   │───▶│ agent-transcripts/*.jsonl       │ │
│  └──────────────┘    └──────────────┬──────────────────┘ │
│  ┌──────────────┐    ┌──────────────▼──────────────────┐ │
│  │ Hooks plugin │───▶│ decipher/events/*.jsonl         │ │
│  └──────────────┘    └──────────────┬──────────────────┘ │
│                                     │                     │
│  ┌──────────────────────────────────▼──────────────────┐ │
│  │ Decipher Extension (Node)                           │ │
│  │  Watcher → TranscriptParser → HookMerger            │ │
│  │         → TemplateEngine                            │ │
...
Local files:
  ...
  decipher/research/<conversation>.json   (per-turn cache)
  decipher/events/<conversation>.jsonl    (hooks)
```
...
- `shared/` — TypeScript types (`SessionState`, `LoadingPhase`, etc.)
- `glossary/`, `knowledge/`, `recommendations/` — static data
- `docs/` — product documentation (this file)

**Stack:** TypeScript, React 18, esbuild, vitest (82 tests)
...
|------|----------|-----------------|
| Transcripts | Cursor-managed `.jsonl` | No (read only) |
| Hook events | `decipher/events/` | No |
| Research cache | `decipher/research/` | No |
...
2. **Hooks recommended for rich output** — without hooks, no command results, diffs, or agent model for witty phrases
...
9. **Loader requires an up-to-date install** — local dev changes are not picked up until `npm run install:cursor` and a window reload
...
## 12. Future roadmap (not in v0.2)

| Phase | Feature |
|-------|---------|
| v0.2 polish | Figma mockups aligned to turn recap + overlay (Activity tab frame still shows old summary strip) |
| v0.3 | Optional minimum splash on open; manual research as default for power users |
| v0.3+ | Cross-conversation browser, community-editable concepts, replay/scrub timeline |
...
- [x] Dogfood validated on real sessions
...
3. Should Decipher publish to Open VSX / Cursor marketplace, or stay sideload-only?
...
## Appendix A: v0.1 → v0.2 changelog (product)
...
| Schema | `shared/activity-schema.ts` (`liveHeadline`, `liveSummary`, `LoadingPhase`) |
